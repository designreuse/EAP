	      ...
	      <div class="col-md-12">
	          <div class="box box-info frame-mini">
	            <div class="box-header with-border">
	              <h3 class="box-title">联系人列表</h3>
	            </div>
	            <div class="box-body">
				  <div class="callout callout-gray">
	              	<c:if test="${user.authority eq 'SUPER'}">
						<div class="btn btn-social btn-success" onclick="add_person_layer()">
							<i class="fa fa-plus"></i>添加联系人
						</div>					
						<div class="btn btn-social btn-warning" onclick="edit_person_select()" >
							<i class="fa fa-edit"></i>编辑联系人
						</div>
						<div class="btn btn-social btn-danger" onclick="remove_person_select()" >
							<i class="fa fa-bitbucket"></i>删除联系人
						</div>				
					</c:if>	
					...
					<table  
				         ...
				         data-toolbar="#toolbar">
				          ...
				          <tr>
				            ...
				            <th data-sortable="true">联系人名称</th>
				      	...
				      	<c:forEach items="${listPersons}" var="person">									
							<tr>
								<c:if test="${user.authority eq 'SUPER'}">
									<td><input type="checkbox" name="checkbox" id="sel_${person.id}" /></td>
								</c:if>
								<td>${person.name}</td>
								<td>${person.organization.name}</td>
								<td>					
								<c:if test="${user.authority eq 'SUPER'}">
									<div class="btn btn-xs btn-social btn-warning" onclick="edit_person_layer('${person.id}')" >
										<i class="fa fa-edit"></i>编辑
									</div>
									<div class="btn btn-xs btn-social btn-danger" onclick="remove_person_layer('${person.id}')">
...
function edit_person_select(){
	...
	if(number==0){
		...
		edit_person_layer(check_id);
	}
}
//批量删除
function remove_person_select(){
	var number=0;
	...
	if(number==0)
		...
		if (confirm(msg)==true){ 
			$("input:checkbox[id^='sel']:checked").each(function(i){	
				...
				check_id=check_id.substring(4);		
				 $.ajax({
			            url: 'admin/remove/person/'+check_id,
			            cache: false,
			            success: function () {			        		
			        		location.reload();	
