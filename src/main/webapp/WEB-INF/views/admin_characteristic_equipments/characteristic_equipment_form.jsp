	...
	<label class="col-sm-2 control-label">联系人：</label>
		<div class="col-sm-10">
			<form:select id="person_select" path="contact.id" class="form-control select2" style="width: 100%;" >	
				<c:if test="${!empty characteristic_equipment.contact.id}">	
					<option selected="selected" value="${characteristic_equipment.contact.id}">${characteristic_equipment.contact.name}</option>
				</c:if>	
			</form:select>
		</div>
	<c:if test="${!empty characteristic_equipment.contact.id}">	
    	<script>getPerson_edit('person_select','${characteristic_equipment.contact.id}');</script>
    </c:if>
    <c:if test="${empty characteristic_equipment.contact.id}">	
		...
		<div class="col-sm-10">
			<form:select path="organization.id" class="form-control select2" style="width: 100%;" >
	            	<c:if test="${!empty characteristic_equipment.organization.id}">
							<option selected="selected" value="${characteristic_equipment.organization.id}">${characteristic_equipment.organization.name}</option>				
						<c:forEach items="${listOrganizations}" var="organization">
							<c:if test="${organization.id ne characteristic_equipment.organization.id}">
