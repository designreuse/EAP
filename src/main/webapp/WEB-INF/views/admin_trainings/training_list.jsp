	      ...
	      <div class="col-md-12">
	          <div class="box box-info frame-mini">
	            <div class="box-header with-border">
	              <h3 class="box-title">教育培训列表</h3>
	            </div>
	            <div class="box-body">
				  <div class="callout callout-gray">
	              	<c:if test="${fn:contains(user.authority_id.training, 'A')}">
						<div class="btn btn-social btn-success" onclick="add_training_layer()">
							<i class="fa fa-plus"></i>添加教育培训
						</div>					
					</c:if>	
					<c:if test="${user.authority eq 'SUPER'}">
						<div class="btn btn-social btn-warning" onclick="edit_training_select()" >
							<i class="fa fa-edit"></i>编辑教育培训
						</div>
						<div class="btn btn-social btn-danger" onclick="remove_training_select()" >
							<i class="fa fa-bitbucket"></i>删除教育培训
						</div>				
					</c:if>	
					<c:if test="${empty user.authority_id.training}">
	            ...
	            <div>
					<table  
				         ...
				         data-toolbar="#toolbar">
				      	...
				      	<tbody>
				      	<c:forEach items="${listTrainings}" var="training">									
							<tr>
								<c:if test="${user.authority eq 'SUPER'}">
									<td><input type="checkbox" name="checkbox" id="sel_${training.id}" /></td>
								</c:if>
								<td>
								...
								<td>${training.issuer.name}</td>
								<td>${fn:substring(training.time, 0, fn:length(training.time)-2)}</td>
								<td>
									<c:if test="${user.authority eq 'ADMIN' && training.organization.id eq user.organization.id}">
										<c:if test="${fn:contains(user.authority_id.training, 'E')}">
											<div class="btn btn-xs btn-social btn-warning" onclick="edit_training_layer('${training.id}')" >
												<i class="fa fa-edit"></i>编辑
											</div>
										</c:if>
										<c:if test="${fn:contains(user.authority_id.training, 'D')}">
											<div class="btn btn-xs btn-social btn-danger" onclick="remove_training_layer('${training.id}')">
											     <i class="fa fa-bitbucket"></i>删除
											</div>
										</c:if>									
										<c:if test="${!fn:contains(user.authority_id.training, 'E')&&!fn:contains(user.authority_id.training, 'D')}">
											<div class="btn btn-xs bg-navy" >
											     无操作权限
											</div>
										</c:if>									
									</c:if>									
									<c:if test="${user.authority eq 'ADMIN' && training.organization.id ne user.organization.id}">
										<div class="btn btn-xs bg-navy" >
											     无操作权限
										</div>								
									</c:if>									
...
function edit_training_select(){
	...
	if(number==0){
		...
		edit_training_layer(check_id);
	}
}
//批量删除
function remove_training_select(){
	var number=0;
	...
	if(number==0)
		...
		if (confirm(msg)==true){ 
			$("input:checkbox[id^='sel']:checked").each(function(i){	
				...
				check_id=check_id.substring(4);		
				 $.ajax({
			            url: 'admin/remove/training/'+check_id,
			            cache: false,
			            success: function () {			        		
			        		location.reload();	
