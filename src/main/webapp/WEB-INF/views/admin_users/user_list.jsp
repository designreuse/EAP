	          ...
	          <div class="box box-info frame-mini">
	            ...
	            <div class="box-body">
				  <div class="callout callout-gray">
	              	<c:if test="${user_login.authority eq 'SUPER'}">
						<div class="btn btn-social btn-success" onclick="add_user_layer()">
							<i class="fa fa-plus"></i>添加用户
						</div>					
						<div class="btn btn-social btn-warning" onclick="edit_user_select()" >
							<i class="fa fa-edit"></i>编辑用户
						</div>
						<div class="btn btn-social btn-danger" onclick="remove_user_select()" >
							<i class="fa fa-bitbucket"></i>删除用户
						</div>				
					</c:if>	
					<c:if test="${user_login.authority ne 'SUPER'}">
					...
					<table  
				         ...
				         data-toolbar="#toolbar">
				          ...
				          <tr>
				            ...
				            <th data-sortable="true">用户昵称</th>
				            <th data-sortable="true">用户组</th>
				            <th data-sortable="true">角色</th>
				            <th data-sortable="true">可用性</th>
				      	...
				      	<c:forEach items="${listUsers}" var="user">									
							<tr>
								<c:if test="${user_login.authority eq 'SUPER'}">
									<td><input type="checkbox" name="checkbox" id="sel_${user.id}" /></td>
								</c:if>
								<td>${user.name}</td>
								<td>${user.username}</td>
								...
								<td>					
								<c:if test="${user_login.authority eq 'SUPER'}">
									<div class="btn btn-xs btn-social btn-warning" onclick="edit_user_layer('${user.id}')" >
										<i class="fa fa-edit"></i>编辑
									</div>
									<div class="btn btn-xs btn-social btn-danger" onclick="remove_user_layer('${user.id}')">
	...
	if(number==0)
		...
		if (confirm(msg)==true){ 
			$("input:checkbox[id^='sel']:checked").each(function(i){	
				...
				check_id=check_id.substring(4);		
				 $.ajax({
			            url: 'admin/remove/user/'+check_id,
			            cache: false,
			            success: function () {			        		
			        		location.reload();	
