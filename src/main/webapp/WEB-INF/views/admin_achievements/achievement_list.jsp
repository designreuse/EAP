	      ...
	      <div class="col-md-12">
	          <div class="box box-info frame-mini">
	            <div class="box-header with-border">
	              <h3 class="box-title">科研成果列表</h3>
	            </div>
	            <div class="box-body">
				  <div class="callout callout-gray">
	              	<c:if test="${fn:contains(user.authority_id.achievement, 'A')}">
						<div class="btn btn-social btn-success" onclick="add_achievement_layer()">
							<i class="fa fa-plus"></i>添加科研成果
						</div>					
					</c:if>	
					<c:if test="${user.authority eq 'SUPER'}">
						<div class="btn btn-social btn-warning" onclick="edit_achievement_select()" >
							<i class="fa fa-edit"></i>编辑科研成果
						</div>
						<div class="btn btn-social btn-danger" onclick="remove_achievement_select()" >
							<i class="fa fa-bitbucket"></i>删除科研成果
						</div>				
					</c:if>	
					<c:if test="${empty user.authority_id.achievement}">
					...
					<table  
				         ...
				         data-toolbar="#toolbar">
				      	...
				      	<c:forEach items="${listAchievements}" var="achievement">									
							<tr>
								<c:if test="${user.authority eq 'SUPER'}">
									<td><input type="checkbox" name="checkbox" id="sel_${achievement.id}" /></td>
								</c:if>
								<td>
						        <div class="herf_font_animal_name">
									<a href="show/achievement/detail/${achievement.id}" target="_blank">												
										${achievement.title}											
									</a>
								</td>		
								<td>${achievement.organization.name}</td>
								<td>${achievement.issuer.name}</td>
								<td>${fn:substring(achievement.time, 0, fn:length(achievement.time)-2)}</td>
								<td>
									<c:if test="${user.authority eq 'ADMIN' && achievement.organization.id eq user.organization.id}">
										<c:if test="${fn:contains(user.authority_id.achievement, 'E')}">
											<div class="btn btn-xs btn-social btn-warning" onclick="edit_achievement_layer('${achievement.id}')" >
												<i class="fa fa-edit"></i>编辑
											</div>
										</c:if>
										<c:if test="${fn:contains(user.authority_id.achievement, 'D')}">
											<div class="btn btn-xs btn-social btn-danger" onclick="remove_achievement_layer('${achievement.id}')">
											     <i class="fa fa-bitbucket"></i>删除
											</div>
										</c:if>									
										<c:if test="${!fn:contains(user.authority_id.achievement, 'E')&&!fn:contains(user.authority_id.achievement, 'D')}">
											<div class="btn btn-xs bg-navy" >
											     无操作权限
											</div>
										</c:if>									
									</c:if>									
									<c:if test="${user.authority eq 'ADMIN' && achievement.organization.id ne user.organization.id}">
										<div class="btn btn-xs bg-navy" >
											     无操作权限
										</div>								
									</c:if>									
...
<script>
//选择编辑
function edit_achievement_select(){
	var number=0;
	var check_id="";
	...
	if(number==0){
		...
		edit_achievement_layer(check_id);
	}
}
//批量删除
function remove_achievement_select(){
	var number=0;
	...
	if(number==0)
		...
		if (confirm(msg)==true){ 
			$("input:checkbox[id^='sel']:checked").each(function(i){	
				...
				check_id=check_id.substring(4);		
				 $.ajax({
			            url: 'admin/remove/achievement/'+check_id,
			            cache: false,
			            success: function () {			        		
			        		location.reload();	
