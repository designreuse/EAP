	            ...
	            <div class="box-body">
				  <div class="callout callout-gray">
	              	<c:if test="${fn:contains(user.authority_id.law, 'A')}">
						<div class="btn btn-social btn-success" onclick="add_law_layer()">
							<i class="fa fa-plus"></i>添加法律法规
						</div>					
					</c:if>	
					<c:if test="${user.authority eq 'SUPER'}">
						<div class="btn btn-social btn-warning" onclick="edit_law_select()" >
							<i class="fa fa-edit"></i>编辑法律法规
						</div>
						<div class="btn btn-social btn-danger" onclick="remove_law_select()" >
							<i class="fa fa-bitbucket"></i>删除法律法规
						</div>				
					</c:if>	
					<c:if test="${empty user.authority_id.law}">
					...
					<table  
				         ...
				         data-toolbar="#toolbar">
				          ...
				          <tr>
				            ...
				            <th data-sortable="true">法律法规标题</th>
				      	...
				      	<tbody>
				      	<c:forEach items="${listLaws}" var="law">									
							<tr>
								<c:if test="${user.authority eq 'SUPER'}">
									<td><input type="checkbox" name="checkbox" id="sel_${law.id}" /></td>
								</c:if>
								<td>
						        <div class="herf_font_animal_name">
									<a href="show/law/detail/${law.id}" target="_blank">												
										${law.title}											
									</a>
								</td>		
								<td>${law.organization.name}</td>
								<td>${law.issuer.name}</td>
								<td>${fn:substring(law.time, 0, fn:length(law.time)-2)}</td>
								<td>
									<c:if test="${user.authority eq 'ADMIN' && law.organization.id eq user.organization.id}">
										<c:if test="${fn:contains(user.authority_id.law, 'E')}">
											<div class="btn btn-xs btn-social btn-warning" onclick="edit_law_layer('${law.id}')" >
												<i class="fa fa-edit"></i>编辑
											</div>
										</c:if>
										<c:if test="${fn:contains(user.authority_id.law, 'D')}">
											<div class="btn btn-xs btn-social btn-danger" onclick="remove_law_layer('${law.id}')">
											     <i class="fa fa-bitbucket"></i>删除
											</div>
										</c:if>									
										<c:if test="${!fn:contains(user.authority_id.law, 'E')&&!fn:contains(user.authority_id.law, 'D')}">
											<div class="btn btn-xs bg-navy" >
											     无操作权限
											</div>
										</c:if>									
									</c:if>									
									<c:if test="${user.authority eq 'ADMIN' && law.organization.id ne user.organization.id}">
										<div class="btn btn-xs bg-navy" >
											     无操作权限
										</div>								
									</c:if>									
...
function edit_law_select(){
	...
	if(number==0){
		...
		edit_law_layer(check_id);
	}
}
//批量删除
function remove_law_select(){
	var number=0;
