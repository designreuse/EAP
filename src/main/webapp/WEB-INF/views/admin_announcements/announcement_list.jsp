	            ...
	            <div class="box-body">
				  <div class="callout callout-gray">
	              	<c:if test="${fn:contains(user.authority_id.announcement, 'A')}">
						<div class="btn btn-social btn-success" onclick="add_announcement_layer()">
							<i class="fa fa-plus"></i>添加公告通知
						</div>					
					</c:if>	
					<c:if test="${user.authority eq 'SUPER'}">
						<div class="btn btn-social btn-warning" onclick="edit_announcement_select()" >
							<i class="fa fa-edit"></i>编辑公告通知
						</div>
						<div class="btn btn-social btn-danger" onclick="remove_announcement_select()" >
							<i class="fa fa-bitbucket"></i>删除公告通知
						</div>				
					</c:if>	
					<c:if test="${empty user.authority_id.announcement}">
						<div class="btn bg-navy" onclick="remove_announcement_select()" >
							无操作权限
						</div>				
					</c:if>	
				</div>
					...
					<table  
				         ...
				         data-toolbar="#toolbar">
				      	...
				      	<c:forEach items="${listAnnouncements}" var="announcement">									
							<tr>
								<c:if test="${user.authority eq 'SUPER'}">
									<td><input type="checkbox" name="checkbox" id="sel_${announcement.id}" /></td>
								</c:if>
								<td>
								...
								<td>
									<c:if test="${user.authority eq 'ADMIN' && announcement.organization.id eq user.organization.id}">
										<c:if test="${fn:contains(user.authority_id.announcement, 'E')}">
											<div class="btn btn-xs btn-social btn-warning" onclick="edit_announcement_layer('${announcement.id}')" >
												<i class="fa fa-edit"></i>编辑
											</div>
										</c:if>
										<c:if test="${fn:contains(user.authority_id.announcement, 'D')}">
											<div class="btn btn-xs btn-social btn-danger" onclick="remove_announcement_layer('${announcement.id}')">
											     <i class="fa fa-bitbucket"></i>删除
											</div>
										</c:if>									
										<c:if test="${!fn:contains(user.authority_id.announcement, 'E')&&!fn:contains(user.authority_id.announcement, 'D')}">
											<div class="btn btn-xs bg-navy" onclick="remove_announcement_layer('${announcement.id}')">
											     无操作权限
											</div>
										</c:if>									
									</c:if>									
									<c:if test="${user.authority eq 'ADMIN' && announcement.organization.id ne user.organization.id}">
										<div class="btn btn-xs bg-navy" onclick="remove_announcement_layer('${announcement.id}')">
											     无操作权限
										</div>								
									</c:if>									
...
<script>
//选择编辑
function edit_announcement_select(){
	var number=0;
	var check_id="";
	...
	if(number==0){
		...
		edit_announcement_layer(check_id);
	}
}
//批量删除
function remove_announcement_select(){
	var number=0;
	...
	if(number==0)
		...
		if (confirm(msg)==true){ 
			$("input:checkbox[id^='sel']:checked").each(function(i){	
				...
				check_id=check_id.substring(4);		
				 $.ajax({
			            url: 'admin/remove/announcement/'+check_id,
			            cache: false,
			            success: function () {			        		
			        		location.reload();	
